...
import { type Message, type MessagePart, type ToolInvocationPart, type ToolInvocation } from "@/components/ui/chat-message"
    ...
    if (lastAssistantMessage.parts && lastAssistantMessage.parts.length > 0) {
      const updatedParts = lastAssistantMessage.parts.map((part: MessagePart) => {
        if (
          ...
          (part as ToolInvocationPart).toolInvocation.state === "call"
        ) {
          needsUpdate = true
          const updatedToolInvocation: ToolInvocation = {
            state: "result",
            toolName: (part as ToolInvocationPart).toolInvocation.toolName,
            result: {
              content: "Tool execution was cancelled",
              __cancelled: true,
            },
          }
          const updatedPart: ToolInvocationPart = {
            ...part,
