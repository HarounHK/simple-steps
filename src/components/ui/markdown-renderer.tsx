...
const CodeBlock = ({
  ...
  const code =
    typeof children === "string" ? children : collectAllText(children)
  ...
  return (
    <div className="group/code relative mb-4">
      <Suspense
        fallback={
          <pre className={preClasses} {...restProps}>
            {children}
          </pre>
        }
      >
        <HighlightedPre language={language} className={preClasses}>
          {code}
        </HighlightedPre>
      </Suspense>
...
function collectAllText(node: ReactNode): string {
  if (typeof node === "string") return node
  if (React.isValidElement(node) && node.props?.children) {
    const child = node.props.children
    return Array.isArray(child)
      ? child.map(collectAllText).join("")
      : collectAllText(child)
  }
  return ""
}
